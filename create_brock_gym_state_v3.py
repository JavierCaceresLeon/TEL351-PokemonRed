...
VERSION 3: Corrige Nombres y OT para que los Pokemon sean visibles.
"""
...
Y_POS = 0xD361
X_POS = 0xD362
FACING_DIR = 0xD364  # 00=Down, 04=Up, 08=Left, 0C=Right
SPRITE_DIR = 0xC109  # Sprite 0 facing direction
...
# Mapeo de caracteres (Pokemon Gen 1 encoding)
CHAR_MAP = {
...
def encode_name(name):
    bytes_list = []
    for char in name:
        bytes_list.append(CHAR_MAP.get(char, 0x7F))
    bytes_list.append(TERMINATOR)
    # Pad with 0x00 or 0x50? Usually just terminator is enough, but fixed length buffers
    while len(bytes_list) < NAME_LENGTH:
        bytes_list.append(0x00) # Or 0x50? Let's use 0x00 after terminator
    return bytes_list[:NAME_LENGTH]

def set_name(pyboy, addr, name):
    encoded = encode_name(name)
    for i, byte in enumerate(encoded):
        write_byte(pyboy, addr + i, byte)
...
def set_pokemon(pyboy, slot, config):
    ...
    # Set Nickname
    nickname_addr = PARTY_NICKNAMES + (slot * NAME_LENGTH)
    set_name(pyboy, nickname_addr, config['nickname'])
    
    # Set OT Name (Use "RED" or similar)
    ot_addr = PARTY_OT_NAMES + (slot * NAME_LENGTH)
    set_name(pyboy, ot_addr, "RED")
...
if __name__ == '__main__':
    ...
    # 5. Posicion (Asegurar)
    # Mover al jugador FRENTE a Brock para facilitar el combate
    # Brock esta en (4, 1). Posicion frente a el: (4, 2)
    write_byte(pyboy, MAP_ID, 54)
    write_byte(pyboy, X_POS, 4)
    write_byte(pyboy, Y_POS, 2)
    
    # Mirar hacia ARRIBA (0x04) para hablar con Brock
    write_byte(pyboy, FACING_DIR, 0x04)
    write_byte(pyboy, SPRITE_DIR, 0x04)
    
    print("  Posicion asegurada en Pewter Gym (54) frente a Brock (4, 2)")
    print("  Mirando hacia ARRIBA (Listo para hablar)")
