"""
Script interactivo para ejecutar el HybridSageAgent entrenado.
Equivalente a run_pretrained_interactive.py pero para tu modelo híbrido.
...
def run_interactive(scenario_id, phase_name, headless=False, max_steps=10000):
    """
    Ejecuta el agente híbrido entrenado en modo interactivo.
    ...
    """
    print(f"\n{'='*60}")
    print(f"  EJECUTANDO HYBRID SAGE AGENT - MODO INTERACTIVO")
    ...
    model_path = project_path / 'models_local' / 'hybrid' / f"{scenario_id}_{phase_name}.zip"
    ...
    agent_wrapper = HybridSageAgent(agent_config)
...
def main():
    parser = argparse.ArgumentParser(
        description='Ejecuta un agente Hybrid Sage entrenado en modo interactivo'
    )
    parser.add_argument(
        '--scenario',
        type=str,
